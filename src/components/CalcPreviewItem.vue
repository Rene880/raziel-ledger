<template>
  <div class="flex flex-row items-center pr-2" :class="localQuantity === max ? 'bg-secondary' : 'bg-primary'">
    <img :src="imgUrl" style="max-height: 50px; max-width: 50px;" class="mr-2">
...
export default {
  ...
  computed: {
    imgUrl() {
      return import.meta.env.BASE_URL + 'img/item/' + this.tag + (this.animated ? '.gif' : '.jpg');
    },
    localQuantity: {
      get() {
