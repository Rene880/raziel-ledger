        ...
        <div :class="displayList === 0 ? 'flex flex-row flex-wrap' : 'grid grid-cols-1 sm:grid-cols-2'">
          <div v-for="bullet in bullets.bullets" class="tooltip-parent">
            <div v-if="displayList === 0">
              <img :src="imgUrl(bullet)" width="60px" :title="bullet.name" :class="getBullet(bullet) > 0 ? '' : 'grayscale-80'">
              <div class="flex flex-row justify-around">
                ...
                <a class="cursor-pointer" @click="addBullet(bullet)">
                  <fa-icon :icon="['fas', 'plus-circle']" class="text-primary text-sm hover:text-link-hover"></fa-icon>
                </a>
              </div>
              <span class="tooltip">
                {{ bullet.name }}
            ...
            <div v-else class="flex flex-row flex-wrap items-center">
              <img :src="imgUrl(bullet)" width="40px" :title="bullet.name" class="ml-4 mr-2" :class="getBullet(bullet) > 0 ? '' : 'grayscale-80'">
