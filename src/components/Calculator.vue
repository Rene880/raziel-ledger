...
export default {
  ...
  emits: [
    'update:unitsProgress',
    'update:unitsSplitMats',
    'update:unitsHideCompletedMats',
    'update:unitsDisplayList'
  ],
  data() {
    return {
      unit_index: -1,
  ...
  methods: {
    addUnit() {
      this.progress[this.unit_index] = new UnitProgress(this.unitsData.materials.length);
      this.unit_index = -1;
    },
    removeUnit(unitKey) {
    ...
    getItemProgressFor(unitKey, item, materialStep) {
      ...
      for (let itemRef of itemRefs) {
        if (this.progress[unitKey].materials[materialStep][itemRef] === undefined) {
          this.progress[unitKey].materials[materialStep][itemRef] = 0;
        }
